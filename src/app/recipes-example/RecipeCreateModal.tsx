...
interface RecipeCreateModalProps {
    open: boolean;
    onClose: () => void;
    onCreated: (recipe: TRecipeDocument) => void;
}

const DIFFICULTIES = ["easy", "medium", "hard"] as const;
const TAG_PATTERN = /^[a-z0-9-]+$/;

async function createRecipe(data: TCreateRecipeInput): Promise<TRecipeDocument> {
...
const DEFAULT_INGREDIENTS: TIngredient[] = [{ name: "", qty: 0, unit: "" }];

export default function RecipeCreateModal({ open, onClose, onCreated }: RecipeCreateModalProps) {
    ...
    return (
        <Dialog open={open} onClose={handleClose} maxWidth="sm" fullWidth scroll="paper">
                ...
                <Stack spacing={2.5}>
                    ...
                    <Autocomplete
                        ...
                        onChange={(_, newValue) => {
                            const valid = newValue
                                ...
                                .filter((t) => t.length >= 2 && t.length <= 20 && TAG_PATTERN.test(t));
                    ...
                    <Box>
                        ...
                        <Stack spacing={1}>
                            ...
                            {steps.length < 30 && (
                                <Button size="small" onClick={addStep} sx={{ alignSelf: "flex-start" }}>
                                    + Add step
                                </Button>
                            )}
                        </Stack>
            ...
            <DialogActions>
                ...
                <Button variant="contained" onClick={handleCreate} disabled={mutation.isPending}>
